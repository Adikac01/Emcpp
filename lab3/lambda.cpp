...
int main(){
  ...
  int center = 50;
  std::sort(v.begin(), v.end(), [center](const int& a, const int& b){
  ...
  auto positiveEvenFirst= [](const int& a, const int& b){
      if (a == 0){
         return b < 0;
      }
      if (b == 0){
  ...
  auto l1_norm = [](auto a){
    return std::accumulate(a.begin(), a.end(), 0.0, [](auto a, auto b){return a + abs(b);});
  }; ;                           // [ 5 ]
